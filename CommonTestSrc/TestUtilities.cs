...
namespace Ionic.Zip.Tests.Utilities
{
    class TestUtilities
    {
        ...
        internal static int GenerateFilesOneLevelDeep(ITestOutputHelper outp,
                                                      string testName,
                                                      string dirToZip,
                                                      ...
                                                      out int subdirCount)
        {
            ...
            return GenerateFilesOneLevelDeep(outp, testName, dirToZip, settings, update, out subdirCount);
        }
                                                      ...
                                                      out int subdirCount)
        {
            ...
            outp.WriteLine("{0}: Creating {1} subdirs.", testName, subdirCount);
            for (int i = 0; i < subdirCount; i++)
            {
                ...
                if (update != null)
                    update(1, filecount);
                outp.WriteLine(":: Subdir {0}, Creating {1} files.", i, filecount);
